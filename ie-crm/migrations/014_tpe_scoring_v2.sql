-- Migration 014: TPE Scoring v2
-- Fixes: 4-tier thresholds, owner age case+fallback, maturity/distress in blend
-- Weight optimizations: ownership cap 30, stress cap 15, owner-user 10, hold_15yr 12, balloon_high 12, lien 7
...
-- New config row for C-tier threshold (4-tier model)
...
-- ============================================================
-- SECTION 2: Recreate property_tpe_scores VIEW with all fixes
-- ============================================================
...
-- FIX: Owner age — case-insensitive role match + fallback to static contacts.age
owner_age AS (
  SELECT DISTINCT ON (pc.property_id)
    pc.property_id,
    COALESCE(
...
scores AS (
  SELECT
    ...
    -- Sale commission estimate (tiered: 3%/2%/1%)
    CASE
    ...
    -- Lease commission estimate (tiered rates by SF)
    CASE
      WHEN p.rba IS NOT NULL AND p.rba > 0 THEN
        p.rba *
        CASE
...
SELECT
  ...
  -- FIX: TPE Score now includes maturity_boost + distress_score (capped at 100)
  ...
  -- FIX: Blended Priority now includes maturity_boost + distress_score
  ROUND(
    LEAST(s.lease_score + s.ownership_score + s.age_score + s.growth_score + s.stress_score
  ...
  -- FIX: 4-tier classification (A/B/C/D) with maturity+distress in the blend
  CASE
