-- Migration 008: TPE Scoring VIEW + Missing Columns
-- Adds: property_tpe_scores VIEW — the competitive advantage weapon
-- Adds: date_of_birth on contacts (for TPE owner age scoring — computed, not estimated)
-- Adds: costar_star_rating on properties (1-5 national building quality metric)
-- Safe to re-run: uses IF NOT EXISTS and DROP VIEW IF EXISTS
-- NOTE: FK references were fixed directly in migration 007 before first run

BEGIN;

-- ============================================================
-- SECTION 1: MISSING COLUMNS FOR TPE
-- ============================================================

-- Owner age comes from contacts.date_of_birth (formula: AGE(NOW(), date_of_birth))
-- The VIEW JOINs through property_contacts (role = 'Owner') to find the oldest owner
ALTER TABLE contacts
  ADD COLUMN IF NOT EXISTS date_of_birth DATE;

-- CoStar Star Rating (1-5 national building quality metric)
ALTER TABLE properties
  ADD COLUMN IF NOT EXISTS costar_star_rating     INTEGER CHECK (costar_star_rating BETWEEN 1 AND 5);

-- Drop old VIEW first (it may reference owner_age_est which we're removing)
DROP VIEW IF EXISTS property_tpe_scores;

-- Drop owner_age_est if it was added (wrong location — age lives on contacts)
ALTER TABLE properties
  DROP COLUMN IF EXISTS owner_age_est;

-- ============================================================
-- SECTION 3: property_tpe_scores VIEW
-- The Transaction Probability Engine — computes 5 scoring models live.
-- All weights come from tpe_config table so adjusting a weight
-- instantly recalculates every property score.
--
-- Model 1: TPE Score (0-100) — lease + ownership + age + growth + stress
-- Model 2: ECV (Estimated Commission Value) — what this deal is worth
-- Model 3: Blended Priority = TPE * 0.70 + ECV_scaled * 0.30
-- Model 4: Maturity Boost — confirmed loan maturity urgency
-- Model 5: Distress Score — auction/NOD/matured loan severity
-- ============================================================
...
SELECT
  ...
  -- Tier classification based on blended priority
  CASE
...
COMMENT ON VIEW property_tpe_scores IS 'Transaction Probability Engine — 5-model live scoring for all properties. Weights from tpe_config table.';
